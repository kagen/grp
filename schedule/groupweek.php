...
if (is_array($hash['user']) && count($hash['user']) > 0) {
	...
	echo '<table class="schedulegroup" style="border-spacing:0;border-collapse:collapse;"><tr><th>&nbsp;</th>';
	...
	foreach ($hash['user'] as $key => $value) {
		...
		for ($i = 0; $i <= 6; $i++) {
			...
			echo sprintf('&nbsp;<a href="add.php?year=%s&month=%s&day=%s&member=%s&caller=%s">＋</a>',$_GET['year'],$_GET['month'],$day,$key,"groupweek.php");
			if (is_array($data[$key][$day]) && count($data[$key][$day]) > 0) {
				foreach ($data[$key][$day] as $row) {
					...
					echo sprintf('<br /><a style="padding:2px;background:%s;color:%s;" href="groupday.php%s"%s>%s%s</a>', $row['schedule_event_bgcolor'], $row['schedule_event_fontcolor'], $parameter, $calendar->share($row), $row['schedule_starttime'], ($row['schedule_event_disp']?"（":"").$row['schedule_event_disp'].($row['schedule_event_disp']?"）":"").$row['schedule_title']);
				}
			}
			echo '&nbsp;</td>';
			$timestamp = strtotime('+1 day', $timestamp);
