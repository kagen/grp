...
class Authority {
	...
	function login() {
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {
			if (strlen($_POST['userid']) > 0) {
				...
				if ($postuserid != '' && count($error) <= 0) {
					...
					$query = sprintf("SELECT id,userid,password,realname,user_group,authority,user_overtime_flg FROM %suser WHERE userid = '%s'", DB_PREFIX, $connection->quote($postuserid));
		...
		if ($authorized === true && count($error) <= 0) {
			...
			$_SESSION['user_overtime_flg'] = $data['user_overtime_flg'];
			$_SESSION['authority'] = $data['authority'];
			$connection = new Connection;
			$query = sprintf("UPDATE %suser SET last_login = CURRENT_TIMESTAMP WHERE userid = '%s'", DB_PREFIX, $connection->quote($postuserid));
