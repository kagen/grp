...
class Validation {
	...
	function url($field, $caption) {
		...
		if (strlen($_POST[$field]) > 0 && !preg_match('/^(https?|file):\/\/[-_a-zA-Z0-9.!~*\'();\/?:@&=+$,%#]+$/', $_POST[$field])) {
			return $caption.'の値が正しくありません。';
		}
	...
	function date($field, $caption, $format = 'Y-m-d H:i:s') {

		$d = DateTime::createFromFormat($format, $_POST[$field]);

		if ($d && $d->format($format) == $_POST[$field]) {
			return $caption.'は正しい日付を入力してください。';
		}
