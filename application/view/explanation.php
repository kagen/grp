...
class Explanation {
	...
	function explain($type) {
		...
		switch ($type) {
			...
			case 'userauthority':
				...
				break;
			case 'dakokutest':
				$string .= 'タイムカード打刻練習用文字列を設定できます。この文字列でQRコード化したカードで練習打刻できます。';
				break;
			case 'timecardcloseday':
				$string .= 'タイムカードの毎月の締め日を設定できます、例えば「10日」を設定すると、当月の11日から翌月の10日までが一つの区切りとして表示・統計されます。';
				break;
			case 'timecardlatenightovertime':
				$string .= '深夜残業の開始時間と終了時間を設定します。';
				break;
			case 'overtime':
				$string .= '残業申請したい時間です。<br />';
				$string .= '例：<br /><table><tr><td>';
				$string .= '1時間30分</td><td>　→　</td><td>1:30</td></tr><tr><td>';
				$string .= '45分</td><td>　→　</td><td>0:45</td></tr></table>';
				$string .= '※休日勤務の場合はそのまま<br />勤務時間を入力してください。';
				break;
			case 'overtimehourlimit':
				$string .= '毎月時間外申請制限時間です。<br />';
				$string .= '残業時間がこの制限を超えると<br />説明（コメント）が必要になります';
				break;
			case 'storageoperate':
				$string .= 'ファイルまたはフォルダーのリネームは右クリック。<br />※リネーム対象の編集権限<br /><br />';
				$string .= 'ファイルまたはフォルダーの移動はドラッグ・アンド・ドロップ<br />※移動対象と元フォルダーの編集権限<br />※移動先フォルダーの書き込み権限';
				break;
		}
	...
	function item($array) {
		...
		if (is_array($array) && count($array) > 0) {
			$string = '<table style="border-spacing:0;border-collapse:collapse;">';
