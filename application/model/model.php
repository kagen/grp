...
class Model extends Connection {
	...
	function findLimit($sort = null, $desc = 0, $searchfield = null, $join = null, $field = null, $distinct = false) {
		...
		if (isset($this->join) && is_array($this->join) && count($this->join > 0)) {
		...
		$query = sprintf("SELECT %s %s FROM %s %s %s %s", ($distinct?"DISTINCT":""), $string, $this->table, $joins, $where, $order);
	...
	function findView($id = 0, $join = null, $field = null) {
		...
		if ($id > 0) {
			...
			$query = sprintf("SELECT %s FROM %s %s WHERE %s.id = %d", $field, $this->table, $join, $this->table, intval($id));
	...
	function insertPost() {
		...
		if (count($this->error) <= 0 && count($this->post) > 0) {
			...
			if (is_array($field) && count($field) > 0) {
				if (!isset($this->post['owner']) || strlen($this->post['owner']) <= 0) {
				...
				foreach ($field as $key) {
					if (array_key_exists($key, $this->post)) {
						$keys[] = $key;
						$values[] = ($this->post[$key] === null?"NULL":"'".$this->quote($this->post[$key])."'");
					}
				}
	...
	function attachment($directory, $prefix, $filename, $type = '') {
		...
		if (file_exists($file)) {
			$filing = new Filing;
			if (preg_match('/.+\.(jpeg|jpg|gif|png)$/i', $filename) && $type != 'attachment') {
	...
	function array_values_recursive($array) {
	    ...
	    foreach ($array as $a) {
	        if (is_array($a)) {
	            $flatarr = $this->array_values_recursive($a);
	        	if (is_array($flatarr)) {
		        	foreach ($flatarr as $e) {
		        		$result[] = $e;
		        	}
	        	} else {
	        		$result[] = $e;
