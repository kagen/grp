...
class Forum extends ApplicationModel {
	...
	function Forum() {
		...
		'forum_comment'=>array('内容', 'notnull', 'length:15000', 'line:200'),
		'forum_date'=>array('fix'=>date('Y-m-d H:i:s'), 'except'=>array('search', 'update')),
		'forum_file'=>array(),
		'forum_parent'=>array('掲示板ID', 'numeric', 'except'=>array('search', 'update')),
	...
	function view() {
		...
		if ($hash['parent']['forum_parent'] == 0) {
			...
			$hash += $this->findLimit('forum_date');
			$hash += $this->findUser($hash['parent']);
			$this->insertAccess();
			$hash['parent']['access_count'] = $this->getAccessCount();
			return $hash;
		} else {
	...
	function commentadd() {
		...
		$parent = $this->permitFind('public', $_POST['forum_parent']);
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {
			...
			$this->permitValidate();
			$this->post['forum_file'] = $this->uploadfile('forum', $_SESSION['userid'].'_'.strtotime($this->post['forum_date']));
			$this->post['folder_id'] =  $parent['folder_id'];
			$this->post['public_level'] =  $parent['public_level'];
			$this->post['public_user'] =  $parent['public_user'];
			$this->post['public_group'] =  $parent['public_group'];
			$this->post['edit_level'] =  $parent['edit_level'];
			$this->post['edit_user'] =  $parent['edit_user'];
			$this->post['edit_group'] =  $parent['edit_group'];
