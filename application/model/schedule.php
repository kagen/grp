...
class Schedule extends ApplicationModel {
	...
	function Schedule() {
		...
		'schedule_event'=>array('except'=>array('search')),
	...
	function scheduleWhere($type = 'month', $data = null) {
		...
		if ($type == 'month') {
			...
			$where = "((schedule_level = 0) OR (schedule_level = 1 AND schedule_name = '%s') OR ";
			$where .= "(schedule_level = 2 AND (schedule_group LIKE '%%[%s]%%' OR schedule_user LIKE '%%[%s]%%')))";
			return sprintf($where, $this->quote($data['userid']), $this->quote($data['userid']), $this->quote($data['user_group']), $this->quote($data['userid']));
		}
	...
	function add() {
		...
		if ($_SERVER['REQUEST_METHOD'] == 'GET') {
			$hash['member'] = (isset($_GET['member'])?$_GET['member']:$_SESSION['userid']);
		}
	...
	function groupweek() {
		...
		$query = "SELECT DISTINCT grp_user.userid, grp_user.realname FROM grp_user LEFT JOIN grp_user_group ON grp_user_group.userid = grp_user.userid WHERE (grp_user.user_group IN (".$this->getChildrenIdsByParentId(intval($hash['groupid'])).") OR grp_user_group.group_id IN (".$this->getChildrenIdsByParentId(intval($hash['groupid'])).")) AND (grp_user.user_retired > CURRENT_TIMESTAMP OR grp_user.user_retired IS NULL OR grp_user.user_retired = '0000-00-00') ORDER BY grp_user.user_order,grp_user.id";
	...
	function findOwner($owner) {
		...
		$query = "SELECT DISTINCT grp_user.userid, grp_user.realname FROM grp_user LEFT JOIN grp_user_group ON grp_user_group.userid = grp_user.userid WHERE (grp_user.user_group IN (".$this->getUserGroups(intval($result['user_group']),$owner).") OR grp_user_group.group_id = ".intval($result['user_group']).") ORDER BY grp_user.user_order, grp_user.id";
		$data = $this->fetchAll($query);
	...
	function holiday() {
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {
			if (isset($_POST['mode']) && $_POST['mode'] == "google") {
				$calendar = new Calendar;
				$startDate = strtotime("-1 month");
				$year = date('Y', $startDate);
				$month = date('n', $startDate);
				$holidays = $calendar->jp_holidays(GOOGLE_API,$year,$month,12);
				if (is_array($holidays) && count($holidays) > 0) {
					foreach ($holidays as $dt => $desc) {
						$query = sprintf("SELECT COUNT(*) AS cnt FROM %sholiday WHERE holiday_date = '%s'", DB_PREFIX, $dt);
						$tmp = $this->fetchOne($query);
						if ($tmp['cnt'] <= 0) {
							$query = sprintf("INSERT INTO %sholiday (holiday_date, holiday_desc) VALUES ('%s','%s')", DB_PREFIX, $dt, $desc);
							$this->response = $this->query($query);
						}
					}
				}
			} elseif (isset($_POST['mode']) && $_POST['mode'] == "add") {
				#TODO ADD validate'
				$query = sprintf("SELECT COUNT(*) AS cnt FROM %sholiday WHERE holiday_date = '%s'", DB_PREFIX, $_POST['holiday_date']);
				$tmp = $this->fetchOne($query);
				if ($tmp['cnt'] > 0) {
					$hash['error'] = array($_POST['holiday_date']."、この日付は既に設定されております");
				} else {
					$query = sprintf("INSERT INTO %sholiday (holiday_date, holiday_desc) VALUES ('%s','%s')", DB_PREFIX, $_POST['holiday_date'], $_POST['holiday_desc']);
					$this->response = $this->query($query);
				}
			} elseif (isset($_POST['mode']) && $_POST['mode'] == "delete") {
				$query = sprintf("DELETE FROM %sholiday WHERE holiday_date = '%s'", DB_PREFIX, $_POST['holiday_date']);
				$this->response = $this->query($query);
			}
			if (!$hash['error']) {
				header('Location:holiday.php');//F5が押されでも再度Form Postさせない為に
			}
		}
		$query = sprintf("SELECT * FROM %sholiday ORDER BY holiday_date DESC", DB_PREFIX);
		$hash['list'] = $this->fetchAll($query);
		return $hash;
	}

	function jsonholidays() {
		$query = sprintf("SELECT * FROM %sholiday ORDER BY holiday_date DESC", DB_PREFIX);
		$holidays = $this->fetchAll($query);
	...
	function json() {
		if ($_SERVER['REQUEST_METHOD'] == 'GET') {
			echo '[{"title": "All Day Event","start": "2017-01-12"},{"title": "Long Event","start": "2017-01-13","end": "2017-01-13"}]';
		}
	}
}
